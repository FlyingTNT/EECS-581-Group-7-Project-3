 ...
 */
function parseClass(elements: HTMLTableRowElement[], index: number): ClassData | null
{
    ...
    if(splitClassName.length <= 1) // If there was no departhement prefix, keep all of it (should never happen)
    {
        className = qualifiedClassName;
    }
    else // Keep all of the text after the first hyphen, retaining all of the hyphens in the {name}
    {
    ...
    const maxCreditHours = creditRange.length <= 1 ? minCreditHours : parseInt(creditRange[1].trim());
...
function unparseTime(time: number): string
{
    ...
    return hour.toString() + ":" + (minutes == 0 ? "00" : minutes == 5 ? "05" : minutes.toString()) + (am ? " AM" : " PM");
}
...
function getCurrentPermutation(state: ScheduleState | null = null): SectionData[] | null
{
    ...
    if(state.currentPermutation < 0)
    {
        return null;
    }

    return state.permutations[state.currentPermutation];
}

/**
 * Gets the sections in the given list that have scheduled times
 * @param sections The sections to filter
 * @returns The sections in the given list that have scheduled times
 */
function getScheduledSections(sections: SectionData[]): SectionData[]
{
    return sections.filter(section => section.times.length > 0);
}

/**
 * Gets the sections in the given list that don't have scheduled times
 * @param sections The sections to filter
 * @returns The sections in the given list that don't have scheduled times
 */
function getUnscheduledSections(sections: SectionData[]): SectionData[]
{
    return sections.filter(section => section.times.length === 0);
}
